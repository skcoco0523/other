    ...
    public function handle()
    {
        ...
        */
        ...
        $subscribe_callback = function ($topic, $message) {
            ...
            make_error_log("subscribeMQTT.log","signa_type:".$signa_type);
            make_error_log("subscribeMQTT.log","mac_addr:".$mac_addr."  type:".$type."  type_num:".$type_num."  ver:".$ver."  data:".$data);

            if ($signa_type == 'device-access') {
                // デバイス登録処理  
                if(is_numeric($type_num) == false){
                    make_error_log("subscribeMQTT.log","config/common.php on not found device_type:".$type);
                ...
                if ($device_list !== null && $device_list->isNotEmpty()) {
                    //登録済みデバイス
                    make_error_log("subscribeMQTT.log","device found...mac_addr:".$device_list[0]->mac_addr);
